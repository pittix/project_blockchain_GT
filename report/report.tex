...
The protocol is in fact designed in such a way that the altruistic path is more convenient for the nodes to pursue, pushing the rate of the selfish nodes close to zero. The collaboration will then turn out to be a \gls{ne} for the game played between all the connected entities.
...
Since this approach gives no substantial penalty to selfish nodes, they can still profit from the network without contributing.
