...
This type of behaviour is pretty common to many decentralized networks, like MANETs \cite{MANETs} and protocols, like BitTorrent or ED2K, where a node that doesn't cooperate via the upload has a limited download capability.\cite{BitTorrent} \cite{ed2k}
...
We used a directed graph, to ignore the simulation of collision events, as we include them in the $p_r$. Another advantage is that we can treat the channel as a M/M/1 queue which will deliver the packet to the following node. Then, the latter will process the packet and enqueue it  until the data is delivered to the destination.
