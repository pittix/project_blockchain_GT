...
The metric can be a combination of parameters, i.e. the success rate, the delay, the bitrate or the \gls{rtt}, that result to a weighting of the network connections graph. Periodically an alive message is sent around the network to inform the neighbour about the presence of an alive node. With another period, the local table of each node is shared around the network to allow the other nodes to determine the weight of the link and the best next hop if they want to send a packet through a node.
If for smaller networks it's feasible for each node to have the informations on how to reach that node, for bigger networks the local table could be set so that the network has a fixed gateway to reach another part of the network. This allows \gls{batman}-adv to run in home routers.
