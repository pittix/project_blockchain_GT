...
Applications are then randomly and uniformly setup between pairs of points, according to a probability coefficient, later called ``app rate''.
...
If the network doesn't apply any punishment, the selfish node is encouraged to act selfishly if the number of selfish node is small, otherwise its bitrate will drop as well. With 100 nodes, if th selfish rate is less than $0.167$ is slightly convenient to act selfishly, over that bound it's slightly convenient to act altruistically.
 A plot of this case is shown in \ref{fig:no-blame}
